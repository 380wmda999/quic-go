...
func (r *RingBuffer[T]) PopFront() T {
	if r.Empty() {
		panic("github.com/380wmda999/quic-go/internal/utils/ringbuffer: pop from an empty queue")
	}
	r.full = false
	t := r.ring[r.headPos]
...
func (r *RingBuffer[T]) PeekFront() T {
	if r.Empty() {
		panic("github.com/380wmda999/quic-go/internal/utils/ringbuffer: peek from an empty queue")
	}
